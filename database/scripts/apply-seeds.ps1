...
if ($LASTEXITCODE -ne 0) { Write-Error "Seed de roles fallo."; exit $LASTEXITCODE }

Write-Host "Ejecutando seed de equipos y partidos de prueba..."
...
if ($LASTEXITCODE -ne 0) { Write-Error "Seed de equipos y partidos fallo."; exit $LASTEXITCODE }

Write-Host "Ejecutando seed de usuarios de prueba..."
Get-Content -Raw .\database\seeds\003_seed_test_users.sql | docker compose exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1'
if ($LASTEXITCODE -ne 0) { Write-Error "Seed de usuarios de prueba fallo."; exit $LASTEXITCODE }
